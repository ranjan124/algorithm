...
public class ArraysExample {
    ...
    public int[] productExceptSelf(int[] num) {
        // 3, 4, 1, 2
        int[] prefixProduct = new int[num.length];
        prefixProduct[0] = 1;
        for (int i = 1; i < num.length; i++) {
            prefixProduct[i] = prefixProduct[i - 1] * num[i - 1];
        }
        int suffixProduct = 1;
        for (int i = num.length - 1; i >= 0; i--) {
            prefixProduct[i] = prefixProduct[i] * suffixProduct;
            suffixProduct = suffixProduct * num[i];
        }
        return prefixProduct;
    }


    public int maximumProductSubArray(int[] array) {
        int value = array[0];
        int min = 1;
        int max = 1;
        for (int i = 0; i < array.length; i++) {
            if (array[i] == 0) {
                ...
                int temp = min;
                min = Math.min(array[i], Math.min(array[i] * min, array[i] * max));
                max = Math.max(array[i], Math.max(array[i] * temp, array[i] * max));
